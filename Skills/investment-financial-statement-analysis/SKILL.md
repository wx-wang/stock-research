...
本项目写作时先读[研究报告写作规范](../../研究规则/研究报告写作规范.md)。正文使用自然、准确的中文；内部字段和计算方法保留在研究记录中，不作为默认报告标题。
...
For a named company, first confirm the listed entity and ticker through the installed Tushare MCP (`mcp__tushareMcp__*`): use `stock_basic` for A-shares and `hk_basic` for Hong Kong listings, after checking the exposed tool schema. Record the market, as-of date, report type, report period, publication date, investor question, available comparison periods, and known missing inputs. Do not confuse a parent group, subsidiary, brand, or concept with the listed entity.

Use the company's filing and notes when the analysis depends on accounting policy, segment definitions, audit opinion, impairments, related parties, commitments, or management explanations. Use the installed Tushare MCP as the default structured source; do not depend on a legacy local fetch script. The following endpoint names apply to A-shares; use the exposed Hong Kong equivalents for Hong Kong companies and verify currency, period and field definitions: `income`, `balancesheet`, `cashflow`, `fina_indicator`, `fina_mainbz`, `forecast`, `express`, and `disclosure_date`; use `daily`, `adj_factor`, and `daily_basic` only when price, shares, or market value are needed for the downstream valuation date. Use `report_rc` for a dated sell-side forecast set and label its broker/sample coverage; it is not automatically complete consensus. Treat broker reports, Knowledge Star, IMA, and media as expectation or narrative evidence, not proof of reported accounting facts.
